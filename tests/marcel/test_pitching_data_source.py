"""Tests for new-style pitching DataSource."""
...
class TestPitchingDataSource:
    """Tests for the PitchingDataSource class."""

    def test_is_callable(self, test_context: Context) -> None:
        """PitchingDataSource is callable."""
        source = PitchingDataSource()
        assert callable(source)

    def test_factory_returns_data_source(self, test_context: Context) -> None:
        """create_pitching_source() returns a PitchingDataSource."""
        source = create_pitching_source()
        assert isinstance(source, PitchingDataSource)
    ...
    def test_all_players_returns_sequence(self, test_context: Context) -> None:
        ...
        with patch("fantasy_baseball_manager.marcel.data_source.pybaseball") as mock_pb:
            ...
            mock_pb.pitching_stats.return_value = mock_df
    ...
    def test_uses_year_from_context(self, test_context: Context) -> None:
        ...
        with patch("fantasy_baseball_manager.marcel.data_source.pybaseball") as mock_pb:
            ...
            source(ALL_PLAYERS)
            mock_pb.pitching_stats.assert_called_with(2025, qual=0)

            # Switch context to different year
            with new_context(year=2023):
                source(ALL_PLAYERS)
                mock_pb.pitching_stats.assert_called_with(2023, qual=0)

    def test_returns_error_on_failure(self, test_context: Context) -> None:
        """Returns Err on pybaseball failure."""
        with patch("fantasy_baseball_manager.marcel.data_source.pybaseball") as mock_pb:
            mock_pb.pitching_stats.side_effect = Exception("Network error")
