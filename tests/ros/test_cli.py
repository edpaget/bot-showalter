...
class TestROSProjectCommand:
    def test_default_shows_both(self) -> None:
        _install_fake()
        result = runner.invoke(app, ["players", "ros-project", "2025"])
        assert result.exit_code == 0
        assert "ROS" in result.output
        assert "batters" in result.output
        assert "pitchers" in result.output

    def test_batting_only(self) -> None:
        _install_fake()
        result = runner.invoke(app, ["players", "ros-project", "2025", "--batting"])
        assert result.exit_code == 0
        assert "batters" in result.output
        assert "pitchers" not in result.output

    def test_pitching_only(self) -> None:
        _install_fake()
        result = runner.invoke(app, ["players", "ros-project", "2025", "--pitching"])
        assert result.exit_code == 0
        assert "pitchers" in result.output
        assert "batters" not in result.output

    def test_engine_flag(self) -> None:
        _install_fake()
        result = runner.invoke(app, ["players", "ros-project", "2025", "--engine", "marcel"])
        assert result.exit_code == 0
