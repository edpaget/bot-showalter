...
def list_cmd() -> None:
    ...
    for meta in models:
        typer.echo(f"  {meta.name} ({meta.player_type}):")
        typer.echo(f"    Training years: {meta.training_years}")
        typer.echo(f"    Stats: {meta.stats}")
...
def info_cmd(
    ...
    typer.echo(f"Model: {meta.name} ({meta.player_type})")
    typer.echo(f"Training years: {meta.training_years}")
    typer.echo(f"Stats: {meta.stats}")
    typer.echo(f"Features: {meta.feature_names}")
    typer.echo(f"Created: {meta.created_at}")

    # Load model to show feature importances
    model_set = store.load(name, player_type)
    typer.echo("\nFeature importances by stat:")
