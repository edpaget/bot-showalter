...
class SqliteDatasetAssembler:
        ...
        with self._provider.connection() as conn:
            for tf in transforms:
                for output in tf.outputs:
                    conn.execute(f"ALTER TABLE [{table_name}] ADD COLUMN [{output}] REAL")
            ...
            for tf in transforms:
                ...
                if tf.weights:
                    ...
                    for group_key, group_rows in groups.items():
                        ...
                        conn.execute(update_sql, [*values, *group_key])
                else:
                    for group_key, group_rows in groups.items():
    ...
    def _create_split(
        ...
        with self._provider.connection() as conn:
            ...
            conn.execute(f"DROP TABLE IF EXISTS [{split_table}]")
            conn.execute(
                f"CREATE TABLE [{split_table}] AS SELECT * FROM [{handle.table_name}] WHERE season IN ({placeholders})",
                seasons,
            )

            row = conn.execute(f"SELECT COUNT(*) FROM [{split_table}]").fetchone()
            row_count: int = row[0]
    ...
    def read(self, handle: DatasetHandle) -> list[dict[str, Any]]:
        with self._provider.connection() as conn:
            cursor = conn.execute(f"SELECT * FROM [{handle.table_name}]")
            columns = [desc[0] for desc in cursor.description]
