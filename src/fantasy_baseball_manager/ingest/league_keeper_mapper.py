...
from fantasy_baseball_manager.domain import LeagueKeeper, PlayerIdentity, PlayerType
...
def import_league_keepers(
    ...
    for row in rows:
        ...
        ptype = PlayerType(player_type_raw) if player_type_raw is not None else None

        player_id: int | None = None

        # Try resolver first, then fall back to name lookup
        if resolver is not None:
            identity = resolver.resolve(raw_name, season=season, player_type=ptype)
        ...
        # Register alias for future lookups (only when type is known)
        if resolver is not None and ptype is not None:
            resolver.register_alias(
                raw_name,
                PlayerIdentity(player_id, ptype),
                source="league_keeper",
            )
