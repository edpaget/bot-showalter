...
2. **Read the roadmap**: use the `mcp__rdm__rdm_roadmap_show` tool with `project: "fbm"` and `roadmap: <slug>` to get the overview and phase list.
...
4. **Read each phase**: use the `mcp__rdm__rdm_phase_show` tool with `project: "fbm"`, `roadmap: <slug>`, and `phase: <stem>` for each phase. Collect titles, bodies, and commit SHAs.
