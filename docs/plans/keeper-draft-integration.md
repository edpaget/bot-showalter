...
| Phase | Status |
|-------|--------|
| 1 — Keeper-adjusted draft pool | not started |
| 2 — Keeper state display | not started |
| 3 — Keeper-aware recommendations | not started |
| 4 — Pre-draft keeper planner | not started |

## Phase 1: Keeper-adjusted draft pool

Remove kept players from the draft pool and re-value the remaining players using the existing `adjust_valuations_for_league_keepers()` pipeline so the draft board reflects true post-keeper scarcity.

### Context

When a league has 12 teams keeping 5 players each, 60 players are removed before the draft starts. This changes replacement level at every position — if 8 of the top 12 shortstops are kept, SS scarcity spikes and the remaining SS should be valued higher. The `compute_adjusted_valuations()` function already does this re-valuation, but the draft session doesn't call it.
...
2. Wire up the keeper-adjusted valuation pipeline: when `keeper_player_ids` is provided, call `adjust_valuations_for_league_keepers()` (or `compute_adjusted_valuations()` directly if the keeper IDs are already known) to re-value the remaining pool, then use those adjusted values in the draft board.
...
5. Inject `KeeperCostRepo` and the projection data needed by the adjusted-valuation pipeline into `SessionManager` via the composition root (`app.py` / factory).
6. Add a convenience mode: when `keeper_player_ids` is not explicitly provided but a `league` parameter is given, auto-derive keepers from `KeeperCostRepo` for that season/league — fetch keeper decisions, take all players with recommendation "keep", and use their IDs.
7. Write tests: starting a session with keeper IDs removes those players from the board; adjusted valuations differ from raw valuations (SS scarcity example); session restore preserves keeper context; auto-derive mode produces the correct keeper set.
...
- When no keeper IDs are provided, behavior is identical to today (backward compatible).

## Phase 2: Keeper state display
...
1. Add a `KeeperInfoType` Strawberry type with fields: `playerId`, `playerName`, `position`, `team` (team name or index), `cost`, `source`.
2. Add a `keepers(sessionId) -> [KeeperInfoType!]!` query to the GraphQL schema. When a session was started with keeper IDs, return the keeper details by joining against `KeeperCostRepo` and `PlayerRepo`.
3. Store the keeper-to-team mapping in the session (extend `DraftSessionRecord` or add a related table). For auto-derived keepers, use `estimate_other_keepers()` to assign keepers to opponent teams; for the user's team, use actual keeper decisions.
...
## Phase 3: Keeper-aware recommendations
...
## Phase 4: Pre-draft keeper planner
...
Phase 1 is the foundation — it wires keeper IDs into the draft session and adjusts valuations. Phase 2 depends on phase 1 for the keeper-player mapping. Phase 3 depends on phases 1-2 for the keeper context and adjusted pool. Phase 4 depends on phase 1 for adjusted valuations and can proceed in parallel with phases 2-3 for the planning UI, though the "start draft" flow requires phase 1.

Phases 1-2 are the minimum viable integration. Phase 3 adds intelligence. Phase 4 is the capstone that connects pre-draft planning to draft execution.
