...
2. **Read the acceptance criteria**:
   - For a phase: use the `mcp__rdm__rdm_phase_show` tool with `project: "fbm"`, `roadmap: <slug>`, and `phase: <number>`.
   - For a task: use the `mcp__rdm__rdm_task_show` tool with `project: "fbm"` and `task: <slug>`.
...
7. **Offer to create rdm tasks** for any actionable issues found using the `mcp__rdm__rdm_task_create` tool with `project: "fbm"`, a descriptive `slug`, `title`, and `body`.
