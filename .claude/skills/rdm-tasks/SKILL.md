...
1. **List tasks**: use the `mcp__rdm__rdm_task_list` tool with `project: "fbm"` to see open and in-progress tasks.
2. **Show details**: if a task slug was provided in `$ARGUMENTS`, use the `mcp__rdm__rdm_task_show` tool with `project: "fbm"` and `task: <slug>`. Otherwise, present the task list and ask the user which task to work on.
3. **Mark in-progress**: use the `mcp__rdm__rdm_task_update` tool with `project: "fbm"`, `task: <slug>`, and `status: "in-progress"`.
